...
**Defined:** 2026-03-15
**Milestone goal:** Audit and eliminate Obj-C/Obj-C++ where possible, add app icon, polish About section, improve restart-tuning UX.

## Swift Migration

- [ ] **SWIFT-01**: Audit all Obj-C/Obj-C++ files — document each file, why it exists, and whether it can be rewritten in Swift
- [ ] **SWIFT-02**: Rewrite `AudioDeviceManager` bridge in pure Swift using CoreAudio C API directly (replace `AudioBridge` SPM target)
- [ ] **SWIFT-03**: Remove `AudioBridge` target from `Package.swift` (3 locations) and clean all references atomically

## App Icon

- [x] **ICON-01**: Create `AppIcon.appiconset` in asset catalog with all 10 required macOS sizes (16x16, 32x32, 64x64, 128x128, 256x256, 512x512, 1024x1024 at 1x and 2x)
- [x] **ICON-02**: Wire `AppIcon` into `Source/Info.plist` via `CFBundleIconName` key
- [x] **ICON-03**: Move `Colors.xcassets` into `Source/Resources/` and update `Package.swift` resources rule to enable asset loading

## About Section

- [x] **ABOUT-01**: Add "Created by Billy Endson" and "© 2026 Billy Endson" credits to `AboutSettings.swift`
- [x] **ABOUT-02**: Update `CFBundleShortVersionString` in `Info.plist` to `1.1`

## Restart Tuning

- [x] **RESTART-01**: Add "Restart" button to `TunerView.swift` in the bottom bar (left side, near "+ Add Custom Tuning") that calls `TunerViewModel.resetTunedStrings()`

## Deferred to v1.2+

- Hot-plug device monitoring (`startDeviceMonitoring()` — no call site in v1.0)
- Scoped audio device selection (replace system-wide HAL default — architectural change)

## v1.1 Traceability

| REQ-ID | Phase | Notes |
|--------|-------|-------|
| SWIFT-01 | 10 | Audit precedes rewrite |
| SWIFT-02 | 10 | CoreAudio C API in Swift |
| SWIFT-03 | 10 | Atomic Package.swift cleanup |
| ICON-01 | 10 | User provides 1024x1024 source image |
| ICON-02 | 10 | Info.plist CFBundleIconName |
| ICON-03 | 10 | xcassets move + Package.swift resources rule |
| ABOUT-01 | 11 | Single-file SwiftUI change |
| ABOUT-02 | 11 | Info.plist version bump |
| RESTART-01 | 11 | UI wiring only — method exists |

*Last updated: 2026-03-15 — Phase 10 complete*
